...
const GameLog = {
    ...
    drawLogs: function() {
        ...
        this.logs.forEach((log, index) => {
            ...
            // Split the message by newlines and wrap long text
            const lines = this.wrapText(log.message, this.canvas.width - 2 * this.padding);
            
            lines.forEach((line, lineIndex) => {
                this.ctx.fillText(line, this.padding, (index + 1 + lineIndex) * (this.logHeight + this.padding));
            });
        });
    },
    ...
    wrapText: function(text, maxWidth) {
        ...
        // Iterate through words and wrap them when necessary
        words.forEach((word) => {
...
document.addEventListener('keydown', function(event) {
    if (event.code === 'Space') {
        ...
        Game.addFood(mouseEvent);
    }

    if (event.code === 'KeyX') {
        document.body.innerHTML = '';
        window.location.href = "https://loilonote.app/_/";
    }
});